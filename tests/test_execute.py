...
class TestExecuteBasics:
    ...
    async def test_execute_returns_int(self, db_client):
        rows = await db_client.execute("SELECT 1")
        assert isinstance(rows, int)

    async def test_execute_insert_returns_one(self, db_client):
        rand = str(uuid.uuid4())
    ...
    async def test_execute_update_returns_correct_count(self, db_client, test_data):
        rows = await db_client.execute(
    ...
    async def test_execute_update_no_match_returns_zero(self, db_client):
        rows = await db_client.execute(
    ...
    async def test_execute_delete_returns_correct_count(self, db_client):
        rand = str(uuid.uuid4())
        await db_client.execute(
            """
            INSERT INTO users (username, email, created_at)
    ...
    async def test_execute_delete_no_match_returns_zero(self, db_client):
        rows = await db_client.execute("DELETE FROM users WHERE id = $1", -9999)
        assert rows == 0

    async def test_execute_multi_row_update(self, db_client, test_data):
...
class TestExecuteVsQuery:
    ...
    async def test_execute_does_not_return_rows(self, db_client, test_data):
    ...
    async def test_query_select_execute_select_same_result(self, db_client):
        ...
        e_result = await db_client.execute("SELECT 1 AS x")
    ...
    async def test_insert_returning_use_query_not_execute(self, db_client):
        ...
        await db_client.execute("DELETE FROM users WHERE id = $1", rows[0]["id"])


class TestExecuteParameterBinding:

    async def test_execute_with_null_parameter(self, db_client, test_data):
        rows = await db_client.execute(
    ...
    async def test_execute_unicode_parameter(self, db_client):
        ...
        await db_client.execute("DELETE FROM users WHERE username = $1", emoji_name)

    async def test_execute_injection_attempt_raises(self, db_client, test_data):
        malicious = f"{test_data['user']['id']} OR 1=1"
        with pytest.raises(oxpg.Error):
            await db_client.execute("DELETE FROM users WHERE id = $1", malicious)


class TestExecuteErrors:

    async def test_execute_syntax_error_raises(self, db_client):
        with pytest.raises(oxpg.Error) as exc:
            await db_client.execute("DELET FROM users WHERE id = $1", 1)
        assert "syntax error" in str(exc.value).lower()

    async def test_execute_missing_column_raises(self, db_client):
        with pytest.raises(oxpg.Error) as exc:
            await db_client.execute(
    ...
    async def test_execute_too_few_args_raises(self, db_client):
        with pytest.raises(oxpg.Error) as exc:
            await db_client.execute(
    ...
    async def test_execute_too_many_args_handled_gracefully(self, db_client):
        try:
            await db_client.execute("SELECT $1::int", 1, 999)
        except Exception:
            pass

    async def test_execute_foreign_key_violation_raises(self, db_client):
        with pytest.raises(oxpg.Error):
            await db_client.execute(
                """
    ...
    async def test_execute_unique_violation_raises(self, db_client):
        rand = str(uuid.uuid4())
        ...
        await db_client.execute("DELETE FROM users WHERE username = $1", username)
